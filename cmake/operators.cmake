...
function(register_cuda_check)
    ...
    foreach(cuop ${register_cuda_check_TARGETS})
            ...
            target_compile_options(${cuop} PRIVATE $<$<COMPILE_LANGUAGE:CUDA>:-dc>)           
            target_link_libraries(${cuop}  ${register_cuda_check_DEPS})
    endforeach()
endfunction(register_cuda_check)

function(register_cuda_check_all)
    set(options "")
    set(oneValueArgs "")
    set(multiValueArgs EXCLUDES DEPS)
    cmake_parse_arguments(register_cuda_check_all "${options}" "${oneValueArgs}"
            "${multiValueArgs}" ${ARGN})

    file(GLOB CUOPS RELATIVE "${CMAKE_CURRENT_SOURCE_DIR}" "*_op.cu")
    string(REPLACE "_mkldnn" "" CUOPS "${CUOPS}")
    string(REPLACE "_cudnn" "" CUOPS "${CUOPS}")
    string(REPLACE "_xpu" "" CUOPS "${CUOPS}")
    string(REPLACE ".cu" "" CUOPS "${CUOPS}")
    list(REMOVE_DUPLICATES CUOPS)
    list(LENGTH register_cuda_check_all_DEPS register_cuda_check_all_DEPS_len)
    foreach(cuop ${CUOPS})
        list(FIND register_cuda_check_all_EXCLUDES ${cuop} _index)
        if (${_index} EQUAL -1)
            set_target_properties(${cuop} PROPERTIES CUDA_SEPARABLE_COMPILATION ON)
            target_compile_options(${cuop} PRIVATE $<$<COMPILE_LANGUAGE:CUDA>:-dc>)
            if (${register_cuda_check_all_DEPS_len} GREATER 0)                    
                target_link_libraries(${cuop}  ${register_cuda_check_all_DEPS})
            endif()
        endif()
    endforeach()
endfunction(register_cuda_check_all)
